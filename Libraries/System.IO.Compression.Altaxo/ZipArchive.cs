...
{
    public partial class ZipArchiveAxo : IDisposable
    {
        private Stream _archiveStream;
        private ZipArchiveEntryAxo _archiveStreamOwner;
        private BinaryReader _archiveReader;
        private ZipArchiveMode _mode;
        private List<ZipArchiveEntryAxo> _entries;
        private ReadOnlyCollection<ZipArchiveEntryAxo> _entriesCollection;
        private Dictionary<string, ZipArchiveEntryAxo> _entriesDictionary;
        ...
        public ZipArchiveAxo(Stream stream) : this(stream, ZipArchiveMode.Read, leaveOpen: false, entryNameEncoding: null) { }
        ...
        public ZipArchiveAxo(Stream stream, ZipArchiveMode mode) : this(stream, mode, leaveOpen: false, entryNameEncoding: null) { }
        ...
        public ZipArchiveAxo(Stream stream, ZipArchiveMode mode, bool leaveOpen) : this(stream, mode, leaveOpen, entryNameEncoding: null) { }
        ...
        public ZipArchiveAxo(Stream stream, ZipArchiveMode mode, bool leaveOpen, Encoding entryNameEncoding)
        {
            if (stream == null)
        ...
        public ReadOnlyCollection<ZipArchiveEntryAxo> Entries
        {
            get
            {
        ...
        public ZipArchiveEntryAxo CreateEntry(string entryName)
        {
        ...
        /// <returns>A wrapper for the newly created file entry in the archive.</returns>
        public ZipArchiveEntryAxo CreateEntry(string entryName, CompressionLevel compressionLevel)
        {
        ...
        public ZipArchiveEntryAxo GetEntry(string entryName)
        {
            if (entryName == null)
        ...
        private ZipArchiveEntryAxo DoCreateEntry(string entryName, CompressionLevel? compressionLevel)
        {
            if (entryName == null)
                throw new ArgumentNullException(nameof(entryName));
            ...
            ZipArchiveEntryAxo entry = compressionLevel.HasValue ?
                new ZipArchiveEntryAxo(this, entryName, compressionLevel.Value) :
                new ZipArchiveEntryAxo(this, entryName);
        ...
        internal void AcquireArchiveStream(ZipArchiveEntryAxo entry)
        {
        ...
        private void AddEntry(ZipArchiveEntryAxo entry)
        {
            _entries.Add(entry);
        ...
        [Conditional("DEBUG")]
        internal void DebugAssertIsStillArchiveStreamOwner(ZipArchiveEntryAxo entry) => Debug.Assert(_archiveStreamOwner == entry);

        internal void ReleaseArchiveStream(ZipArchiveEntryAxo entry)
        {
        ...
        internal void RemoveEntry(ZipArchiveEntryAxo entry)
        {
        ...
        private void Init(Stream stream, ZipArchiveMode mode, bool leaveOpen)
        {
            ...
            try
            {
                ...
                _entries = new List<ZipArchiveEntryAxo>();
                _entriesCollection = new ReadOnlyCollection<ZipArchiveEntryAxo>(_entries);
                _entriesDictionary = new Dictionary<string, ZipArchiveEntryAxo>();
                _readEntries = false;
                ...
                switch (mode)
                {
                    ...
                    default:
                        ...
                        else
                        {
                            ReadEndOfCentralDirectory();
                            EnsureCentralDirectoryRead();
                            foreach (ZipArchiveEntryAxo entry in _entries)
                            {
        ...
        private void ReadCentralDirectory()
        {
            try
            {
                ...
                {
                    AddEntry(new ZipArchiveEntryAxo(this, currentHeader));
                    numberOfEntries++;
                }
            ...
            {
                throw new InvalidDataException(SR.CentralDirectoryInvalid + " " + ex.Message);
            }
        }
        ...
        private void WriteFile()
        {
            ...
            if (_mode == ZipArchiveMode.Update)
            {
                List<ZipArchiveEntryAxo> markedForDelete = new List<ZipArchiveEntryAxo>();
                foreach (ZipArchiveEntryAxo entry in _entries)
                {
                    if (!entry.LoadLocalHeaderExtraFieldAndCompressedBytesIfNeeded())
                        markedForDelete.Add(entry);
                }
                foreach (ZipArchiveEntryAxo entry in markedForDelete)
                    entry.Delete();
