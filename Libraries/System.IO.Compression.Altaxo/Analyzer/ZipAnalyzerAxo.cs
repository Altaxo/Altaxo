...
namespace System.IO.Compression
{
    ...
    public class ZipAnalyzerAxo
    {
        ...
        protected static bool TryGetEndOfCentralDirectoryRecord(Stream str, out EndOfCentralDirectoryRecord? endOfCentralDirectory)
        {
        ...
        public static ZipAnalyzerResult Analyze(Stream str)
        {
            ...
            // now, get all the local files listed in the central directory
            var commonList = new List<(CentralDirectoryRecord CDE, LocalFileHeader LFH)>();
            for (int i = 0; i < listCentralDirectory.Count; ++i)
            {
                var entry = listCentralDirectory[i];
                str.Seek(entry.RelativeOffsetToLocalFileHeader, SeekOrigin.Begin);
                signature = ReadInt32(str);
                if (signature != LocalFileHeader.LocalFileHeaderSignature)
                {
                    result.AreLocalFileHeadersMissing = true;
                    result.MissingLocalFileHeaders.Add(entry);
                    stb.AppendLine($"Signature of local file header at address 0x{entry.RelativeOffsetToLocalFileHeader:X} is incorrect: 0x{signature}");
                }
                var localFileHeader = LocalFileHeader.Create(str);
                commonList.Add((entry, localFileHeader));
            }

            var corruptLocalFileHeadersHashSet = new HashSet<LocalFileHeader>();
            for (int i = 0; i < commonList.Count; ++i)
            {
                if (!IsEqual(commonList[i].LFH, commonList[i].CDE))
                {
                    result.CorruptLocalFileHeaders.Add(commonList[i]);
                    corruptLocalFileHeadersHashSet.Add(commonList[i].LFH);
                    stb.AppendLine($"Local file header of CDE[{i}] is corrupt, Name: {commonList[i].CDE.FileName}");
                }
            ...
            {
                var lfh = entry.LFH;
                if (corruptLocalFileHeadersHashSet.Contains(lfh))
                    continue;

                str.Seek(lfh.StreamPositionOfContentBegin, SeekOrigin.Begin);


                try
                {
                    var partialStream = new ReadOnlyPartialStreamView(str, lfh.StreamPositionOfContentBegin, lfh.CompressedSize);
                    uint checksum = 0;
