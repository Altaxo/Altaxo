...
namespace WeifenLuo.WinFormsUI
{
	...
	public class FloatWindow : Form, IDockListContainer
	{
		...
		private void InternalConstruct(DockPanel dockPanel, DockPane pane, bool boundsSpecified, Rectangle bounds)
		{
			...
			if (pane == null)
				throw(new ArgumentNullException(ResourceHelper.GetString("FloatWindow.Constructor.NullPane")));
		...
		protected override void WndProc(ref Message m)
		{
			...
			else if (m.Msg == (int)Win32.Msgs.WM_NCLBUTTONDBLCLK)
			{
				...
				foreach(DockPane pane in DockList)
				{
					...
					if (pane.DockWindow != null)
					{
						pane.DockState = pane.DockWindow.DockState;
						pane.Activate();
					}
				}
