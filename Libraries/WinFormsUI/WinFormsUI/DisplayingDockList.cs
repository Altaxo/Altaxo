	...
	public class DisplayingDockList : ReadOnlyCollectionBase
	{
		...
		internal void Refresh()
		{
			...
			for (int i=0; i<DockList.Count; i++)
			{
				...
				NestedDockingStatus status = pane.GetNestedDockingStatus(IsFloat);
		...
		private void CalculateBounds()
		{
			...
			this[0].GetNestedDockingStatus(IsFloat).SetDisplayingBounds(Container.DisplayingRectangle, Container.DisplayingRectangle, Rectangle.Empty);
			...
			for (int i=1; i<Count; i++)
			{
				...
				NestedDockingStatus statusPrev = prevPane.GetNestedDockingStatus(IsFloat);
