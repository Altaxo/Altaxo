...
<xsl:template name="type-syntax">
	<pre class="syntax">
		...
		<xsl:text>&#160;</xsl:text>
		<xsl:if test="local-name() != 'interface' and @isAbstract = '1'">abstract&#160;</xsl:if>
		<xsl:choose>
...
<xsl:template name="get-param-list">
	(<xsl:for-each select="parameter">
		<xsl:choose>
			<xsl:when test="contains(@type, ../../@name)"><xsl:value-of select="substring-after(@type, concat(../../@name, '.'))"/></xsl:when>
