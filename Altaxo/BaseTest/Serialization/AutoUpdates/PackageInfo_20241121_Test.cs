...
namespace Altaxo.Serialization.AutoUpdates
{
  namespace Before20241120
  {
    public static class SystemRequirements
    {
      ...
      public static bool MatchesRequirements(PackageInfo packageInfo)
      {
        ...
        // For the test purpose here, we assume that we always have net framework installed
        /* 
    ...
    public class PackageInfo_20241121_Test
    {
      public static readonly string Test_Lines =

        // the preferrable version is the version that uses the installed dotnet 9
        // Note that for backward compatibility it is neccessary to leave the property 'RequiredNetFrameworkVersion' for some time in, until
        // all installations have the new version of the update downloader installed (otherwise, the missing 'RequiredNetFrameworkVersion' property
        // would cause an exception)
        "Unstable\t4.8.3228.0\t253216686\tB9898BE3301B057DA6B1BA41A52A98F6C232F08C\tRequiredNetFrameworkVersion=4.8\tRequiredOperatingSystem=Windows_10.0.17763\tRequiredDotNetVersion=9.0\tFileName=AltaxoBinaries-Win-Net9.0-4.8.3228.0.zip\r\n" +
        // then next is the version that runs on a self contained dotnet
        "Unstable\t4.8.3228.0\t253216686\tB9898BE3301B057DA6B1BA41A52A98F6C232F08C\tRequiredNetFrameworkVersion=4.8\tRequiredOperatingSystem=Windows_10.0.17763\tRequiredArchitecture=X64\tFileName=AltaxoBinaries-Win-x64-4.8.3228.0.zip\r\n" +
        // the last is the obsolete net framework 4.8
        "Unstable\t4.8.3227.0\t253216686\tB9898BE3301B057DA6B1BA41A52A98F6C232F08C\tRequiredNetFrameworkVersion=4.8\r\n"
        ;

      [Fact]
      public void TestBackwardCompatibility()
      {
        var buffer = System.Text.Encoding.UTF8.GetBytes(Test_Lines);
        var memstream = new MemoryStream(buffer);
        var packages = PackageInfo.FromStream(memstream);
        var pkg = PackageInfo.GetHighestVersion(packages);

        Assert.NotNull(pkg);
        Assert.Equal(new Version(4, 8, 3227, 0), pkg.Version);
      }
    }
  }
  ...
  {
    [Fact]
    public void TestForwardCompatibility_DotNet9IsNotInstalled()
    {
      var buffer = System.Text.Encoding.UTF8.GetBytes(Before20241120.PackageInfo_20241121_Test.Test_Lines);
      var memstream = new MemoryStream(buffer);
      var packages = PackageInfo.FromStream(memstream);
      PackageInfo pkg;
      // inject service
      ...
      Assert.NotNull(pkg);
      Assert.Equal("AltaxoBinaries-Win-x64-4.8.3228.0.zip", pkg.PackageFileName);
    }

    [Fact]
    public void TestForwardCompatibility_DotNet9IsInstalled()
    {
      var buffer = System.Text.Encoding.UTF8.GetBytes(Before20241120.PackageInfo_20241121_Test.Test_Lines);
      var memstream = new MemoryStream(buffer);
      var packages = PackageInfo.FromStream(memstream);
      ...
      Assert.NotNull(pkg);
      Assert.Equal("AltaxoBinaries-Win-Net9.0-4.8.3228.0.zip", pkg.PackageFileName);
    }
