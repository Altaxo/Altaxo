...
namespace Altaxo.Collections
{
  ...
  public class ConcurrentTokenizedPriorityQueue<TKey, TValue, TToken> where TKey : IComparable<TKey> where TToken : notnull
  {
    ...
    /// <summary>
    /// Represents an item in the priority queue, holding key, value, and heap position.
    /// </summary>
    private struct Item
    {
      internal TKey Key;
      ...
      internal Item ModifyHeapPosition(int i) { HeapPosition = i; return this; }
      internal Item ModifyKey(TKey key) { Key = key; return this; }
      internal Item ModifyValue(TValue value) { Value = value; return this; }
      /// <inheritdoc/>
    ...
    private int _count;
    /// <summary>
    /// Elements of the priority queue organized as binary heap. Each element doesn't hold the item directly. Instead, it holds a token that is used to retrieve the item from <see cref="_tokenToItem"/> dictionary.
    /// </summary>
    private TToken[] _heap;
    /// <summary>
    /// Dictionary that associates the tokens with the items. The items hold key and value as well as an integer value that designates the position in the <see cref="_heap"/> heap.
    /// </summary>
    ...
    /// <summary>
    /// Function that creates a new token. Argument is the old token. Return value should be a new token, which is different from all tokens created so far.
    /// </summary>
