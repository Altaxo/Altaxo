...
{
  ///<summary>Computes a dot product of a conjugated vector with another vector.</summary>
  ///<remarks> Returns <c> result = x^H * y = conjg(x[1])*y[1] + conjg(x[2])*y[2] + ... + conjg(x[n])*y[n]</c></remarks>
  [System.Security.SuppressUnmanagedCodeSecurityAttribute]
  internal sealed class Dotc 
  {
    private Dotc() {}
...
#else
      dna_blas_zdotc(n, X, incx, Y, incy, ref ret);
#endif
      return ret;
    }
