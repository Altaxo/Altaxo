                ...
                if (srcIndicesLength > 0)
                {
                    ...
                    if (jidx >= 0) // diagonal element is included in src matrix
                    {
                        for (int i = 0; i < srcIndicesLength; ++i)
                        {
                            ...
                            result.indices[row][i] = idx;
                            result.items[row][i] = f(this.items[row][i], row, idx);
                        }
                        result.count[row] = srcIndicesLength;
                    }
                    else // diagonal element is not included in src matrix
                    {
                        int upTo = Math.Min(srcIndicesLength, -jidx);
                        ...
                        // now the diagonal
                        result.indices[row][upTo] = row;
                        result.items[row][upTo] = f(0, row, row);

                        // now elements right of the diagonal
                        for (int i = upTo; i < srcIndicesLength; ++i)
                        {
                            int idx = srcIndices[i];
                            result.indices[row][i + 1] = idx;
                            result.items[row][i + 1] = f(this.items[row][i], row, idx);
                        }
                    }
                }
