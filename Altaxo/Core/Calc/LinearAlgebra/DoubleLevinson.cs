...
namespace Altaxo.Calc.LinearAlgebra
{
	...
	/// </para>
	/// </example>
	sealed public class DoubleLevinson : Algorithm
	{
		#region Fields
		...
		public DoubleMatrix U
		{
			get
			{
				...
				// copy jagged array into a DoubleMatrix
				DoubleMatrix Upper = new DoubleMatrix(m_Order);
#if MANAGED
				for (int i = 0; i < m_Order; i++)
				{
      ...
      for (i = 1; i < m_Order; i++)
      {
		...
		public static DoubleMatrix Inverse(IReadOnlyList<double> col, IReadOnlyList<double> row)
		{
			...
			DoubleMatrix I = new DoubleMatrix(order);           // the solution matrix
			double A1, B1;
      ...
      for (i = 1; i < order; i++)
      {
		...
		public static DoubleVector Solve(IReadOnlyList<double> col, IReadOnlyList<double> row, IReadOnlyList<double> Y)
		{
			...
			double[] B = new double[order];
			double[] Z = new double[order];
			DoubleVector X = new DoubleVector(order);
			double Q, S, Ke, Kr, e;
			double Inner;
			int i, j, l;
		...
		/// </para>
		/// </remarks>
		public static DoubleMatrix Solve(IReadOnlyList<double> col, IReadOnlyList<double> row, IROMatrix Y)
		{
			// check parameters
			if (col == null)
			{
