...
{
  /// <summary>
  /// A Levinson solver for square Toeplitz systems of <c>double</c> type.
  /// </summary>
  ...
  /// linear systems associated with the matrix (see <see cref="Solve(IReadOnlyList{double})"/> members).
  /// <para>
  ...
  /// to solve a symmetric Toeplitz system, use the <see cref="DoubleSymmetricLevinson"/> class,
  ...
  /// </para>
  /// </example>
  public sealed class DoubleLevinson : Algorithm
  {
    #region Fields
    ...
    /// </summary>
    private readonly DoubleVector m_TopRow;
    ...
    /// </summary>
    private double[][] m_UpperTriangle;
    ...
    public DoubleMatrix U
    {
      get
      {
        ...
        // copy jagged array into a DoubleMatrix
        var Upper = new DoubleMatrix(m_Order);
#if MANAGED
        for (int i = 0; i < m_Order; i++)
        {
    ...
    /// </overloads>
    /// <summary>
    /// Constructor with <c>DoubleVector</c> parameters.
    /// </summary>
    /// <param name="col">The left-most column of the Toeplitz matrix.</param>
    /// <param name="row">The top-most row of the Toeplitz matrix.</param>
    /// <exception cref="ArgumentNullException">
    ...
    protected override void InternalCompute()
    {
      ...
      for (i = 1; i < m_Order; i++, AP = A, BP = B)
      {
        ...
        // diagonal matrix
        m_Diagonal[i] = m_Diagonal[i - 1] / (1.0 - Ke * Kr);
      }
    }
    ...
    /// </summary>
    public DoubleVector GetLeftColumn()
    {
    ...
    /// </summary>
    public DoubleVector GetTopRow()
    {
      return new DoubleVector(m_TopRow);
    }
    ...
    public DoubleMatrix GetInverse()
    {
      ...
      double[] B = m_UpperTriangle[m_Order - 1];
      double A1, B1, scale;
      int i, j, k, l;
#if MANAGED
    ...
    public static DoubleMatrix Inverse(IReadOnlyList<double> col, IReadOnlyList<double> row)
    {
      ...
      var I = new DoubleMatrix(order);           // the solution matrix
      double A1, B1;
    ...
    public static DoubleMatrix Solve(IReadOnlyList<double> col, IReadOnlyList<double> row, IROMatrix<double> Y)
    {
      ...
      for (i = 1; i < order; i++)
      {
        ...
        for (l = 0; l < Y.RowCount; l++)
        {
          ...
          var M = DoubleVector.GetColumn(Y, l);
