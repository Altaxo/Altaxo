...
namespace Altaxo.Calc.LinearAlgebra {
  ...
  public sealed class FloatSVDDecomp : Algorithm 
  {
		...
		///<summary>Returns the singular values as a <c>FloatVector</c>.</summary>
		///<returns>the singular values as a diagonal <c>FloatVector</c>.</returns>
		public FloatVector S {
			get{
				Compute();
				return s;
			}
		...
		protected override void InternalCompute(){
			...
			while(m > 0) { // quit if all the singular values have been found.
		...
		private static float dsign(float z1, float z2) {
			float ret=System.Math.Abs(z1)*(z2/System.Math.Abs(z2));
			return ret;
		}

		private static void dswap(FloatMatrix A, int Col1, int Col2) {
		...
		private static void drot (FloatMatrix A, int Col1, int Col2, float c, float s){
