...
// calculate an upper bound, paru, for the zero of the function
      ...
      paru = gnorm / delta;
      if (paru == 0.0)
        paru = double.Epsilon / Math.Min(delta, p1);
...
L150:
      ...
      if (par == 0.0)
        par = Math.Max(double.Epsilon, p001 * paru);
    ...
    //
    {
      ...
      { // LELLID!!
        ...
        { // LELLID!!
          ...
          if (sdiag[k] == 0.0)
            goto L70;
          if (Math.Abs(r[k + k * r_dim1]) >= Math.Abs(sdiag[k]))
            goto L40;
...
// the corresponding diagonal element of r
      ...
      for (j = 0; j < n; ++j)
      {
        if (sdiag[j] == 0.0 && nsing == n)
          nsing = j; // LELLID!! (statt nsing = j-1;)
        if (nsing < n)
          wa[j] = 0.0;
      }
    ...
    private static void qrfac(int m, int n, double[] a, int lda, bool pivot, int[] ipvt,
...
// LELLID: ajnorm = enorm( m - j + 1, a, j + j * a_dim1);
        ...
        { // LELLID
          ...
          if (0.05 * sqr(rdiag[k] / wa[k]) > DBL_EPSILON)
            goto L80;
  ...
  for (i = 0; i < j ; i++)
{
  ...
  return GSL_SUCCESS;
    ...
    public static int ComputeCovariances(LMFunction fcn, double[] x, int n, int m, double[] covar, out double sumchisq, out double sigmasq)
    {
      ...
      int result = LEVMAR_COVAR(jactjac, covar, sumchisq, m, n, out sigmasq);
