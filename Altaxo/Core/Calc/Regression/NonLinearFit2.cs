...
namespace Altaxo.Calc.Regression
{
  ...
  public class NonLinearFit2
  {
    ...
    private const int LM_OPTS_SZ = 5;
    private const int LM_INFO_SZ = 9;
    private const double LM_INIT_MU = 1E-03;
    private const double LM_STOP_THRESH = 1E-17;
    private const double LM_DIFF_DELTA = 1E-06;
    private const double LM_REAL_MAX = double.MaxValue;
    private const double LM_REAL_MIN = double.Epsilon; // this is ok, in C++ this is the smallest number near zero
    private const double LM_REAL_EPSILON = Altaxo.Calc.DoubleConstants.DBL_EPSILON;
    private const double EPSILON = 1E-12;
    private const double ONE_THIRD = 1.0 / 3.0;

    private const int __BLOCKSZ__ = 32; /* block size for cache-friendly matrix-matrix multiply. It should be
                              ...
                              */
    private const int __BLOCKSZ__SQ = (__BLOCKSZ__) * (__BLOCKSZ__);
 ...
 */
                      ...
                      */
    {
      ...
      for (k = stop = 0; k < itmax && 0 == stop; ++k)
      {
        ...
        {
          if (jacTe_inf < (tmp = Math.Abs(jacTe[i])))
            jacTe_inf = tmp;
 ...
 */
    ...
    private static int AX_EQ_B_LU(double[] A, double[] B, double[] x, int m)
    {
      int i, j, k;
      int[] idx;
