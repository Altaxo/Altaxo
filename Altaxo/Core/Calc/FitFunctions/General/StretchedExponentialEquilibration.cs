...
namespace Altaxo.Calc.FitFunctions.General
{
  ...
  [FitFunctionClass]
  public record StretchedExponentialEquilibration : IFitFunctionWithDerivative, IImmutable
  {
    ...
    [Altaxo.Serialization.Xml.XmlSerializationSurrogateFor("AltaxoBase", "Altaxo.Calc.FitFunctions.General.StretchedExponentialEquilibration", 0)]
    ...
    private class XmlSerializationSurrogate0 : Altaxo.Serialization.Xml.IXmlSerializationSurrogate
    {
      ...
      {
        var s = (StretchedExponentialEquilibration)o;
    ...
    public void Evaluate(ReadOnlySpan<double> independent, ReadOnlySpan<double> parameters, Span<double> dependent)
    {
      ...
      if (x > 0)
      {
        ...
        {
          sum += parameters[i] * (1 - Math.Exp(-Math.Pow(x / parameters[i + 1], parameters[i + 2])));
        }
      }
      dependent[0] = sum;
    }
    ...
    public void Evaluate(IROMatrix<double> independent, IReadOnlyList<double> parameters, IVector<double> dependent, IReadOnlyList<bool>? dependentVariableChoice)
    {
      ...
      for (int r = 0; r < rowCount; ++r)
      {
        ...
        if (arg > 0)
        {
          for (int i = 2, j = 0; j < NumberOfTerms; i += 3, ++j)
          {
            sum += parameters[i] * (1 - Math.Exp(-Math.Pow(arg / parameters[i + 1], parameters[i + 2])));
          }
        }
        dependent[r] = sum;
      }
      ...
      for (int r = 0; r < rowCount; ++r)
      {
        ...
        if (x > 0)
        {
          ...
          for (int i = 2; i < parameters.Count; i += 3)
          {
            ...
            DF[r, i] = 1 - earg;
            DF[r, i + 1] = -parameters[i] * earg * arg * beta / tau;
            DF[r, i + 2] = parameters[i] * earg * arg * Math.Log(x / tau);
