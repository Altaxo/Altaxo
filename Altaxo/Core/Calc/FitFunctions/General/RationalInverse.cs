...
namespace Altaxo.Calc.FitFunctions.General
{
        ...
        : IFitFunctionWithDerivative, IImmutable
  {
    ...
    [Altaxo.Serialization.Xml.XmlSerializationSurrogateFor("AltaxoBase", "Altaxo.Calc.FitFunctions.General.RationalInverse", 0)]
    ...
    public string ParameterName(int i)
    {
      if (i < 0 || i >= NumberOfParameters)
        throw new ArgumentOutOfRangeException($"Parameter index {i} out of range [0..{NumberOfParameters - 1}]");
      else
    ...
    public void Evaluate(ReadOnlySpan<double> independent, ReadOnlySpan<double> parameters, Span<double> dependent)
    {
      ...
      {
        nominator = parameters[_order_n - 1];
      ...
      double denominator = parameters[_order_n + _order_m];
