...
namespace Altaxo.Calc.FitFunctions.Chemistry
{
  ...
  public record MassBasedFloryDistributionOfMPeak : IFitFunctionWithDerivative, IFitFunctionPeak, IImmutable
  {
    ...
    private const string ParameterBaseName1 = "MPeak";
    ...
    /// <summary>
    /// 2026-05-29 Initial version
    /// </summary>
    [Serialization.Xml.XmlSerializationSurrogateFor(typeof(MassBasedFloryDistributionOfMPeak), 0)]
    private class XmlSerializationSurrogate0 : Serialization.Xml.IXmlSerializationSurrogate
    {
      ...
      {
        var s = (MassBasedFloryDistributionOfMPeak)o;
      ...
      public virtual object Deserialize(object? o, Serialization.Xml.IXmlDeserializationInfo info, object? parent)
      {
        ...
        return new MassBasedFloryDistributionOfMPeak(numberOfTerms, orderOfBackgroundPolynomial) { IndependentVariableIsDecadicLogarithm = independentVariableIsDecadicLogarithm };
      }
    }
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="MassBasedFloryDistributionOfMPeak"/> class with default values.
    /// </summary>
    public MassBasedFloryDistributionOfMPeak()
    {
      NumberOfTerms = 1;
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="MassBasedFloryDistributionOfMPeak"/> class with the specified number of terms and order of background polynomial.
    /// </summary>
    ...
    public MassBasedFloryDistributionOfMPeak(int numberOfTerms, int orderOfBackgroundPolynomial)
    {
    ...
    [FitFunctionCreator("MassBasedFloryDistributionOfMPeak", "Chemistry", 1, 1, NumberOfParametersPerPeak)]
    [System.ComponentModel.Description("${res:Altaxo.Calc.FitFunctions.Chemistry.MassBasedFloryDistributionOfMPeak}")]
    public static IFitFunction Create_1_M1()
    {
      return new MassBasedFloryDistributionOfMPeak(1, -1);
    }
    ...
    /// <param name="MPeak">The molecular mass of the peak position.</param>
    ...
    public static double GetYOfOneTerm(double M, double area, double MPeak)
    {
      var xx = 2 * M / MPeak;
    ...
    public void EvaluateDerivative(IROMatrix<double> independent, IReadOnlyList<double> parameters, IReadOnlyList<bool>? isFixed, IMatrix<double> DF, IReadOnlyList<bool>? dependentVariableChoice)
    {
      ...
      for (var r = 0; r < rowCount; ++r)
      {
        ...
        for (int i = 0, j = 0; i < NumberOfTerms; ++i, j += NumberOfParametersPerPeak)
        {
          ...
          var MPeak = parameters[j + 1];

          var argtau = 2 * x / MPeak;
          var body = _lnOf10 * argtau * argtau * Math.Exp(-argtau);

          DF[r, j + 0] = body;
          DF[r, j + 1] = body * 2 * area * (x - MPeak) / (MPeak * MPeak);
        }
    ...
    public (IReadOnlyList<double?>? LowerBounds, IReadOnlyList<double?>? UpperBounds) GetParameterBoundariesForPositivePeaks(double? minimalPosition = null, double? maximalPosition = null, double? minimalFWHM = null, double? maximalFWHM = null)
    {
      ...
      for (int i = 0, j = 0; i < NumberOfTerms; ++i, j += NumberOfParametersPerPeak)
      {
        ...
        {
          lowerBounds[j + 1] = IndependentVariableIsDecadicLogarithm ? Math.Pow(10, minimalPosition.Value) : minimalPosition.Value;
        }
        if (minimalPosition.HasValue)
        {
          upperBounds[j + 1] = IndependentVariableIsDecadicLogarithm ? Math.Pow(10, maximalPosition.Value) : maximalPosition.Value;
        }
      }
    ...
    public (double Position, double Area, double Height, double FWHM) GetPositionAreaHeightFWHMFromSinglePeakParameters(IReadOnlyList<double> parameters)
    {
      ...
      const double leftXX = 0.38062011467720152971;
      const double rightXX = 2.0779604501004529510;

      var area = parameters[0];
      var MPeak = parameters[1];
      ...
      if (IndependentVariableIsDecadicLogarithm)
      {
        position = Math.Log10(MPeak);
        width = Math.Log10(rightXX) - Math.Log10(leftXX);
      }
      else
      {
        position = MPeak;
        width = (rightXX - leftXX) * MPeak;
      }
    ...
    public (double Position, double PositionStdDev, double Area, double AreaStdDev, double Height, double HeightStdDev, double FWHM, double FWHMStdDev) GetPositionAreaHeightFWHMFromSinglePeakParameters(IReadOnlyList<double> parameters, IROMatrix<double>? cv)
    {
      ...
      else
      {
        ...
        {
          positionVar = Math.Sqrt(cv[1, 1]);
          widthVar = (rightXX - leftXX) * Math.Sqrt(cv[1, 1]);
        }
      }
