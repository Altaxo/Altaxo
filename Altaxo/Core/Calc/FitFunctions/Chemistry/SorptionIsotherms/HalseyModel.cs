...
{
  /// <summary>
  /// Implements the mass uptake of a sample in dependence of the water activity according to the Halsey model.
  /// </summary>
  /// <remarks>
  /// References:
  ...
  /// </remarks>
  [FitFunctionClass]
  public record HalseyModel : IFitFunction, IFitFunctionWithDerivative, Main.IImmutable
  {
    /// <inheritdoc/>
    ...
    [Altaxo.Serialization.Xml.XmlSerializationSurrogateFor(typeof(HalseyModel), 0)]
    private class XmlSerializationSurrogate : Altaxo.Serialization.Xml.IXmlSerializationSurrogate
    {
    ...
    /// <summary>
    /// Creates the default Halsey model fit function.
    /// </summary>
    /// <returns>A new instance of <see cref="HalseyModel"/>.</returns>
    [FitFunctionCreator("Mass uptake Halsey model", "Chemistry/SorptionIsotherms", 1, 1, 3)]
    [System.ComponentModel.Description("${res:Altaxo.Calc.FitFunctions.Chemistry.SorptionIsotherms.HalseyModel}")]
    ...
    /// <summary>
    /// Evaluates the mass uptake according to the Halsey model.
    /// </summary>
    ...
    {
      return offset + (x == 0 ? 0 : Math.Pow(-C / Math.Log(x), 1 / n));
    }
    ...
    public void Evaluate(IROMatrix<double> independent, IReadOnlyList<double> parameters, IVector<double> dependent, IReadOnlyList<bool>? dependentVariableChoice)
    {
      ...
      {
        dependent[i] = Evaluate(independent[i, 0], parameters[0], parameters[1], parameters[2]);
      }
    }
    ...
    public void EvaluateDerivative(IROMatrix<double> independent, IReadOnlyList<double> parameters, IReadOnlyList<bool>? isFixed, IMatrix<double> DF, IReadOnlyList<bool>? dependentVariableChoice)
    {
      for (int i = 0; i < independent.RowCount; i++)
      {
        ...
        var pw1 = Math.Pow(-C / Math.Log(x), 1 / n);

        DF[i, 0] = 1; // Derivative w.r.t. offset
        DF[i, 1] = pw1 / (C * n); // w.r.t. C
        DF[i, 2] = -pw1 * Math.Log(-C / Math.Log(x)) / (n * n); // w.r.t. n
      }
    }
  }
}
