...
{
  /// <summary>
  /// Implements the mass uptake of a sample in dependence of the water activity according to the Henderson model.
  /// </summary>
  /// <remarks>
  /// References:
  ...
  /// </remarks>
  [FitFunctionClass]
  public record HendersonModel : IFitFunction, IFitFunctionWithDerivative, Main.IImmutable
  {
    /// <inheritdoc/>
    ...
    [Altaxo.Serialization.Xml.XmlSerializationSurrogateFor(typeof(HendersonModel), 0)]
    private class XmlSerializationSurrogate0 : Altaxo.Serialization.Xml.IXmlSerializationSurrogate
    {
      public virtual void Serialize(object o, Altaxo.Serialization.Xml.IXmlSerializationInfo info)
      {
        var s = (HendersonModel)o;
      }

      public virtual object Deserialize(object? o, Altaxo.Serialization.Xml.IXmlDeserializationInfo info, object? parent)
      {
        return new HendersonModel();
      }
    }
    ...
    /// <summary>
    /// Evaluates the mass uptake according to the Henderson model.
    /// </summary>
    ...
    {
      return offset + Math.Pow(-Math.Log(1 - x) / C, 1 / n);
    }
    ...
    public void EvaluateDerivative(IROMatrix<double> independent, IReadOnlyList<double> parameters, IReadOnlyList<bool>? isFixed, IMatrix<double> DF, IReadOnlyList<bool>? dependentVariableChoice)
    {
      for (int i = 0; i < independent.RowCount; i++)
      {
        ...
        var pw1 = Math.Pow(-Math.Log(1 - x) / C, 1 / n);

        DF[i, 0] = 1; // Derivative w.r.t. offset
        DF[i, 1] = -pw1 / (C * n); // w.r.t. C
        DF[i, 2] = -pw1 * Math.Log(-Math.Log(1 - x) / C) / (n * n); // w.r.t. n
      }
    }
  }
}
