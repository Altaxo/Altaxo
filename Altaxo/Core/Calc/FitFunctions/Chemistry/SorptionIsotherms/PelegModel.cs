...
{
  /// <summary>
  /// Implements the mass uptake of a sample in dependence of the water activity according to the Peleg model.
  /// </summary>
  /// <remarks>
  /// References:
  ...
  /// </remarks>
  [FitFunctionClass]
  public record PelegModel : IFitFunction, IFitFunctionWithDerivative, Main.IImmutable
  {
    /// <inheritdoc/>
    ...
    [Altaxo.Serialization.Xml.XmlSerializationSurrogateFor(typeof(PelegModel), 0)]
    ...
    /// <summary>
    /// Creates the default Peleg model fit function.
    /// </summary>
    /// <returns>A new instance of <see cref="PelegModel"/>.</returns>
    [FitFunctionCreator("Mass uptake Peleg model", "Chemistry/SorptionIsotherms", 1, 1, 5)]
    [System.ComponentModel.Description("${res:Altaxo.Calc.FitFunctions.Chemistry.SorptionIsotherms.PelegModel}")]
    ...
    {
      return offset + C1 * Math.Pow(x, C3) + C2 * Math.Pow(x, C4);
    }
    ...
    public void Evaluate(IROMatrix<double> independent, IReadOnlyList<double> parameters, IVector<double> dependent, IReadOnlyList<bool>? dependentVariableChoice)
    {
      ...
      {
        dependent[i] = Evaluate(independent[i, 0], parameters[0], parameters[1], parameters[2], parameters[3], parameters[4]);
      }
    }
    ...
    {
      return ([null, double.Epsilon, double.Epsilon, double.Epsilon, 1], null);
    }
    ...
    public void EvaluateDerivative(IROMatrix<double> independent, IReadOnlyList<double> parameters, IReadOnlyList<bool>? isFixed, IMatrix<double> DF, IReadOnlyList<bool>? dependentVariableChoice)
    {
      for (int i = 0; i < independent.RowCount; i++)
      {
        ...
        double C3 = parameters[3];
        double C4 = parameters[4];

        DF[i, 0] = 1; // Derivative w.r.t. offset
        DF[i, 1] = Math.Pow(x, C3); // w.r.t. C1
        DF[i, 2] = Math.Pow(x, C4); // w.r.t. C2
        DF[i, 3] = C1 * Math.Pow(x, C3) * Math.Log(x);
        DF[i, 4] = C2 * Math.Pow(x, C4) * Math.Log(x);
      }
    }
  }
}
