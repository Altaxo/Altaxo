...
namespace Altaxo.Calc.FitFunctions.Chemistry.SorptionIsotherms
{
  ...
  public record IglesiasChirifeModel : IFitFunction, IFitFunctionWithDerivative, Main.IImmutable
  {
    ...
    /// <param name="p">The model intercept term in the exponential expression.</param>
    /// <param name="b">The model slope term that scales the water-activity dependence.</param>
    ...
    public void Evaluate(IROMatrix<double> independent, IReadOnlyList<double> parameters, IVector<double> dependent, IReadOnlyList<bool>? dependentVariableChoice)
    {
      ...
      {
        dependent[i] = Evaluate(independent[i, 0], parameters[0], parameters[1], parameters[2], parameters[3]);
      }
    }
