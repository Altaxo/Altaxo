...
namespace Altaxo.Calc.FitFunctions.Relaxation
{
  ...
  /// </summary>
  [FitFunctionClass]
  public record HavriliakNegamiModulusRelaxation : IFitFunction, Main.IImmutable
  {
    ...
    private class XmlSerializationSurrogate0 : Altaxo.Serialization.Xml.IXmlSerializationSurrogate
    {
      ...
      {
        var s = (HavriliakNegamiModulusRelaxation)o;
    ...
    /// </summary>
    [Altaxo.Serialization.Xml.XmlSerializationSurrogateFor("AltaxoBase", "Altaxo.Calc.FitFunctions.Relaxation.HavriliakNegamiModulusRelaxation", 1)]
    ...
    public void Evaluate(ReadOnlySpan<double> independent, ReadOnlySpan<double> parameters, Span<double> dependent)
    {
      ...
      Complex64T result = parameters[1] + (parameters[0] - parameters[1]) * Kohlrausch.ReIm(parameters[3], w_r);

      if (_useFlowTerm)
      {
        if (_invertViscosity)
          result = 1 / ((1 / result) - Complex64T.ImaginaryOne * parameters[4] / (x));
        else
          result = 1 / ((1 / result) - Complex64T.ImaginaryOne / (x * parameters[4]));
      }
