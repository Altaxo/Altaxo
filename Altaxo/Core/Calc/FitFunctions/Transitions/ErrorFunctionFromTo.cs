...
namespace Altaxo.Calc.FitFunctions.Transitions
{
  ...
  public record ErrorFunctionFromTo : IFitFunctionWithDerivative, Main.IImmutable
  {
    ...
    /// </summary>
    [Serialization.Xml.XmlSerializationSurrogateFor(typeof(ErrorFunctionFromTo), 0)]
    private class XmlSerializationSurrogate0 : Serialization.Xml.IXmlSerializationSurrogate
    {
      ...
      public virtual object Deserialize(object? o, Serialization.Xml.IXmlDeserializationInfo info, object? parent)
      {
        ...
        return new ErrorFunctionFromTo(order_l, order_r);
      }
    }
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="ErrorFunctionFromTo"/> class, with both polynomial orders set to zero.
    /// </summary>
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="ErrorFunctionFromTo"/> class.
    /// </summary>
    ...
    /// </exception>
    public ErrorFunctionFromTo(int polynomialOrderLeft, int polynomialOrderRight)
    {
    ...
    {
      return new ErrorFunctionFromTo(0, 0);
    }
    ...
    public ErrorFunctionFromTo WithPolynomialOrderLeft(int polynomialOrderLeft)
    {
      ...
      {
        return new ErrorFunctionFromTo(polynomialOrderLeft, _order_r);
      }
      else
      {
        return this;
    ...
    public ErrorFunctionFromTo WithPolynomialOrderRight(int polynomialOrderRight)
    {
      ...
      {
        return new ErrorFunctionFromTo(_order_l, polynomialOrderRight);
      }
      else
      {
        return this;
    ...
    /// function profile.
    /// </summary>
    /// <remarks>This function produces a continuous transition from the left to the right polynomial,
    /// centered at xc, with the width of the transition determined by sigma. It is commonly used for
    /// modeling gradual transitions or soft thresholds.</remarks>
    ...
    /// <param name="sigma">The standard deviation that controls the width of the transition region. Must be positive.</param>
    ...
    public static double Evaluate(double x, double xc, double sigma, ReadOnlySpan<double> coeffs_left, ReadOnlySpan<double> coeffs_right)
    {
      var arg = (x - xc) / (sigma * Sqrt2);
      // for reason of numerical stability we use Erfc here, because it really goes to zero for large arguments
      var transa = 0.5 * ErrorFunction.Erfc(arg);
      var transb = 0.5 * ErrorFunction.Erfc(-arg);
    ...
    public void EvaluateDerivative(IROMatrix<double> independent, IReadOnlyList<double> parameters, IReadOnlyList<bool>? isFixed, IMatrix<double> DF, IReadOnlyList<bool>? dependentVariableChoice)
    {
      ...
      for (int i = 0; i < independent.RowCount; i++)
      {
        ...
        var transa = 0.5 * ErrorFunction.Erfc(arg);
        var transb = 0.5 * ErrorFunction.Erfc(-arg);
        double expTerm = Math.Exp(-arg * arg) / (sigma * Sqrt2Pi);
        ...
        DF[i, 0] = expTerm * (suma - sumb); // Derivative with respect to "xc"
        DF[i, 1] = expTerm == 0 ? 0 : expTerm * arg * (suma - sumb) * Sqrt2; // Derivative with respect to "sigma"
    ...
    public (IReadOnlyList<double?>? LowerBounds, IReadOnlyList<double?>? UpperBounds) GetParameterBoundariesHardLimit()
    {
      ...
      lb[1] = double.Epsilon; // sigma must be > 0
      return (lb, null); // sigma must be > 0
    }
