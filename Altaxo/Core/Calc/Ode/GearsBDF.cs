...
namespace Altaxo.Calc.Ode
{
	...
	public class GearsBDF
	{
		...
		private class NordsieckState
		{
			...
			{
				var relativeTime = (time - tn - dt) / dt; // relativeTime should be in the range [-1..0], because time should be <= (tn+dt)
				var zndata = zn.data; // direct access to zn
				for (int i = 0; i < y_result.Length; ++i)
				{
					double sum = zndata[i][qn];
				...
				)
			{
				...
				VectorMath.Copy(currstate.xn, xcurr);
				var x0 = currstate.xn;
