...
L30:
        ...
        for (int k = 1; k <= mm1; ++k)
        {
          ...
          t = te / (fm - fk);
          s += t;
          if (Math.Abs(t) < eps_d9gmic * Math.Abs(s))
            goto L50;
        }
...
L20:
        ...
        if (e * Math.Abs(aeps) > eps_GammaIC)
          goto L30;
    ...
    private class _LnGamma
    {
      ...
      public static Complex LnGamma(Complex zin, bool bDebug)
      {
        ...
        if (x >= 0.0 && cabsz > _LnGamma_bound)
          goto L50;
        if (x < 0.0 && Math.Abs(y) > _LnGamma_bound)
          goto L50;

        if (cabsz < _LnGamma_bound)
          goto L20;
