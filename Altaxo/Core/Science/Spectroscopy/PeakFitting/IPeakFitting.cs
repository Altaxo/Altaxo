...
{
  /// <summary>
  /// Interface to peak fitting methods.
  /// </summary>
  public interface IPeakFitting
  {
    ...
    /// Description of the peaks (output of peak searching algorithms, see <see cref="PeakSearching.IPeakSearching"/>).
    /// </param>
    /// <param name="cancellationToken">Token used to cancel this task.</param>
    /// <returns>
    /// The results of the peak fitting.
    /// For each spectral region, a tuple containing the fitted peak data in that region and the start and end index (exclusive)
    /// of that region is returned.
    ...
    (
      ...
      double[] y,
      int[]? regions,
      IReadOnlyList<(IReadOnlyList<PeakSearching.PeakDescription> PeakDescriptions, int StartOfRegion, int EndOfRegion)> peakDescriptions,
