...
namespace Altaxo.Calc.Interpolation
{
  ...
  public class BivariateAkimaSplineTests
  {
    ...
    private double[] _refx = { 2.5, 7.5, 12.5, 17.5, 22.5, 27.5, 32.5, 37.5, 42.5, 47.5 };
    private double[] _refy = { 2.5, 7.5, 12.5, 17.5, 22.5, 27.5, 32.5, 37.5 };
    ...
    public void Test1()
    {
      ...
      for (int iy = 0; iy < _testy.Length; iy++)
      {
        for (int ix = 0; ix < _testx.Length; ix++)
        {
          ...
          BivariateAkimaSplineAlgorithm474.Interpolate(_testx, _testy, z, u, v, w);
    ...
    public void TestDerivatives()
    {
      ...
      var zMatrix = MatrixMath.ToROMatrixFromColumnMajorLinearArray(z, _testx.Length);
      var splineNoCache = new BivariateAkimaSpline(_testx, _testy, zMatrix, precomputePatches: false);
      var splineCache = new BivariateAkimaSpline(_testx, _testy, zMatrix, precomputePatches: true);

      var xs = new[] { 12.5, 17.5, 22.5, 27.5 };
      var ys = new[] { 12.5, 17.5, 22.5, 27.5 };
      const double h = 1e-4;
      ...
      foreach (var x in xs)
      {
        foreach (var y in ys)
        {
          var (z1, dzdx1, dzdy1) = splineNoCache.GetValueAndDerivativesOfXY(x, y);
          var (z2, dzdx2, dzdy2) = splineCache.GetValueAndDerivativesOfXY(x, y);

          AssertEx.AreEqual(z1, z2, 1e-12, 1e-12, $"z at x={x}, y={y}");
          AssertEx.AreEqual(dzdx1, dzdx2, 1e-12, 1e-12, $"dzdx at x={x}, y={y}");
          AssertEx.AreEqual(dzdy1, dzdy2, 1e-12, 1e-12, $"dzdy at x={x}, y={y}");

          var dzdxFd = (splineCache.GetValueOfXY(x + h, y) - splineCache.GetValueOfXY(x - h, y)) / (2 * h);
          var dzdyFd = (splineCache.GetValueOfXY(x, y + h) - splineCache.GetValueOfXY(x, y - h)) / (2 * h);

          AssertEx.AreEqual(dzdxFd, dzdx2, 1e-2, 1e-2, $"finite difference dzdx at x={x}, y={y}");
          AssertEx.AreEqual(dzdyFd, dzdy2, 1e-2, 1e-2, $"finite difference dzdy at x={x}, y={y}");
        }
      }
    }
    ...
    public void CompareNewWithOld()
    {
      ...
      foreach (var cached in new bool[] { false, true })
      {
        var splineNew = new BivariateAkimaSpline(_testx, _testy, MatrixMath.ToROMatrixFromColumnMajorLinearArray(z, _testx.Length), precomputePatches: cached);
        ...
        for (int iy = 0; iy < _testy.Length; iy++)
        {
          for (int ix = 0; ix < _testx.Length; ix++)
          {
            ...
            BivariateAkimaSplineAlgorithm474.Interpolate(_testx, _testy, z, u, v, w);
            var wNew = splineNew.GetValueOfXY(u[0], v[0]);
            AssertEx.AreEqual(w[0], wNew, 1E-14, 1e-14);
          }
        }
