...
namespace Altaxo.Serialization.NicoletSPA
{
  ...
  public record NicoletSPAImporter : IDataFileImporter, Main.IImmutable
  {
    ...
    public string? Import(string[] filenames, Altaxo.Data.DataTable table, NicoletSPAImportOptions importOptions)
    {
    ...
    {
      return (new string[] { ".spa" }, "Nicolet/Thermo Omnic files");
    }

    public double GetProbabilityForBeingThisFileFormat(string fileName)
    {
      double p = Path.GetExtension(fileName).ToLowerInvariant() == ".spa" ? 0.5 : 0;

      try
      {
        using var stream = new FileStream(fileName, FileMode.Open, FileAccess.Read, FileShare.Read);
        var result = new NicoletSPAImportResult(stream);
        p += string.IsNullOrEmpty(result.ErrorMessages) ? 0.5 : 0.3;
