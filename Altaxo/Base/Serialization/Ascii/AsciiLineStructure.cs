...
namespace Altaxo.Serialization.Ascii
{
  ...
  /// </summary>
  public class AsciiLineStructure : Main.IImmutable, IEquatable<AsciiLineStructure>
  {
    /// <summary>
    /// The structure of the line. This list holds <see cref="System.Type" /> values that represent the recognized items in the line.
    /// </summary>
    public ImmutableArray<AsciiColumnInfo> RecognizedTypes = ImmutableArray<AsciiColumnInfo>.Empty;

    protected int _hashValue;

    protected int _priorityValue;

    public AsciiLineStructure(ImmutableArray<AsciiColumnInfo> columns)
    {
      RecognizedTypes = columns;
      _hashValue = ((IStructuralEquatable)RecognizedTypes).GetHashCode(EqualityComparer<AsciiColumnInfo>.Default);
      _priorityValue = RecognizedTypes.Sum(x => x.ScoreValue);
    }

    public AsciiLineStructure(IEnumerable<AsciiColumnType> columnTypes, int? count)
    {
    ...
    private class XmlSerializationSurrogate0 : Altaxo.Serialization.Xml.IXmlSerializationSurrogate
    {
      public virtual void Serialize(object obj, Altaxo.Serialization.Xml.IXmlSerializationInfo info)
      {
        ...
        info.CreateArray("ColumnTypes", s.RecognizedTypes.Length);
        for (int i = 0; i < s.RecognizedTypes.Length; ++i)
        {
          info.AddEnum("e", s.RecognizedTypes[i].ColumnType);
        }
        info.CommitArray();
      }
      ...
      public object Deserialize(object? o, Altaxo.Serialization.Xml.IXmlDeserializationInfo info, object? parent)
      {
        ...
        return new AsciiLineStructure(b, b.Length);
      }
    }
    ...
    public bool Equals(AsciiLineStructure? other)
    {
      return (other is { } o) &&
              this._hashValue == o._hashValue &&
             ((IStructuralEquatable)this.RecognizedTypes).Equals(o.RecognizedTypes);
    }

    public override bool Equals(object? obj)
    {
      return Equals(obj as AsciiLineStructure);
    }
    ...
    public bool IsCompatibleWith(AsciiLineStructure ano)
    {
      ...
      {
        if (!IsCompatibleWith(RecognizedTypes[i].ColumnType, ano.RecognizedTypes[i].ColumnType))
          return false;
      }
      return true;
    }
    ...
    public override string ToString()
    {
      ...
      foreach (var entry in RecognizedTypes)
      {
        stb.Append(' ');
