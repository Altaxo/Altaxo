...
namespace Altaxo.Gui.Graph
{
	...
	public class LayerPositionController : ILayerPositionViewEventSink,  IMVCAController
	{
		...
		HostLayer _linkedLayer;

		public LayerPositionController(HostLayer layer)
		{
			_layer = layer;
			SetElements(true);
		}
		...
		public void SetElements(bool bInit)
		{
			...
			if(bInit)
			{
				...
				_scale     = _layer.Scale;
				_clipDataToFrame = _layer is XYPlotLayer ? ((XYPlotLayer)_layer).ClipDataToFrame == LayerDataClipping.StrictToCS : false;
		...
		public bool Apply()
		{
			...
			try
			{
				...
				_layer.Scale    = _scale;
				if(_layer is XYPlotLayer)

				((XYPlotLayer)_layer).ClipDataToFrame = _clipDataToFrame?LayerDataClipping.StrictToCS:LayerDataClipping.None;
		...
		{
			XYPlotLayer oldLinkedLayer = _linkedLayer as XYPlotLayer;
