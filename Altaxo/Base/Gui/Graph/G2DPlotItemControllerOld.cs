...
namespace Altaxo.Gui.Graph
{
	...
	public class G2DPlotItemControllerOld
		:
		...
		IMVCANController
	{
		...
		private IPlotGroupCollectionViewSimple _plotGroupView;

		private IMVCAController _dataController;
		private IMVCAController _plotGroupController;
		...
		private Common.MultiChildController _combinedScatterLineGroupController;

		public G2DPlotItemControllerOld()
		{
		}

		public G2DPlotItemControllerOld(G2DPlotItem doc)
			: this(doc, null)
		{
		}

		public G2DPlotItemControllerOld(G2DPlotItem doc, PlotGroupStyleCollection parent)
		{
		...
		private void InitializeCollectionAndData()
		{
		...
		private void InitializePlotGroupView()
		{
			bool bStandard = true;
			bool bSerial = false;
			bool color = false;
			...
			if (_groupStyles != null)
			{
				...
				if (color && linestyle && typeof(LineStyleGroupStyle) == _groupStyles.GetTypeOfChild(typeof(ColorGroupStyle)))
				{
					bSerial = true;
				}
				if ((linestyle && symbol) && typeof(SymbolShapeStyleGroupStyle) == _groupStyles.GetTypeOfChild(typeof(LineStyleGroupStyle)))
				{
					if (color && !bSerial)
						bStandard = false;
					else
			...
			{
				_plotGroupView = (IPlotGroupCollectionViewSimple)Current.Gui.FindAndAttachControlTo(this, typeof(IPlotGroupCollectionViewSimple));
		...
		private void EhAdvancedPlotGroupControlRequired(object sender, EventArgs e)
		{
			// _plotGroupView.AdvancedPlotGroupControl -= EhAdvancedPlotGroupControlRequired;
			ApplyPlotGroupView();
			_plotGroupView = null;
		...
		/// </remarks>
		private bool UseCombinedScatterLineGroupTab()
		{
			bool useCombinedTab;
			...
			if (_tempdoc.Style.Count > 0)
			{
				...
				if (useCombinedTab)
				{
					...
					if (addHelperStyle)
					{
						...
						{
							ScatterPlotStyle scatterStyle = new ScatterPlotStyle(_doc.GetPropertyContext());
						...
						{
							LinePlotStyle lineStyle = new LinePlotStyle(_doc.GetPropertyContext());
		...
		private void MakeAdditionalPlotStylePermanent()
		{
