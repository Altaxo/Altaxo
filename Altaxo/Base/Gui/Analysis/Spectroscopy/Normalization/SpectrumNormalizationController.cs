...
namespace Altaxo.Gui.Analysis.Spectroscopy.Normalization
{
  ...
  public class NormalizationController : MVCANControllerEditImmutableDocBase<INormalization, INormalizationView>
  {
    ...
    protected override void Initialize(bool initData)
    {
      ...
      if (initData)
      {
        ...
        var methodTypes = new List<Type>(ReflectionService.GetNonAbstractSubclassesOf(typeof(INormalization)));
    ...
    {
      _doc = (INormalization)Activator.CreateInstance(newMethodType);
      CreateSubController();
    }
    ...
    public override bool Apply(bool disposeController)
    {
      if(SubController is not null)
      {
        if (!SubController.Apply(disposeController))
          ...
          _doc = (INormalization)SubController.ModelObject;
      }
