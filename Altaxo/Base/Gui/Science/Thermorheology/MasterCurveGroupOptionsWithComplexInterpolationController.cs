...
namespace Altaxo.Gui.Science.Thermorheology
{
  ...
  public class MasterCurveGroupOptionsWithComplexInterpolationController : MVCANControllerEditImmutableDocBase<MasterCurveGroupOptionsWithComplexInterpolation, IMasterCurveGroupOptionsWithComplexInterpolationView>
  {
    ...
    public DimensionfulQuantity FittingWeightIm
    {
      ...
      set
      {
        if (!(_fittingWeightIm == value))
        {
          _fittingWeightIm = value;
          OnPropertyChanged(nameof(FittingWeightIm));
        }
      }
    }
    ...
    public bool IsParticipatingInFitIm
    {
      ...
      set
      {
        if (!(_isParticipatingInFitIm == value))
        {
          _isParticipatingInFitIm = value;
          OnPropertyChanged(nameof(IsParticipatingInFitIm));
          if (value == false)
          {
            FittingWeightIm = new DimensionfulQuantity(0, FittingWeightIm.PrefixedUnit);
          }
          else if (!(FittingWeightIm.AsValueInSIUnits > 0))
          {
            FittingWeightIm = new DimensionfulQuantity(1, Altaxo.Units.Dimensionless.Unity.Instance).AsQuantityIn(FittingWeightIm.PrefixedUnit);
          }
        }
      }
    }
    ...
    protected override void Initialize(bool initData)
    {
      ...
      if (initData)
      {
        ...
        IsParticipatingInFitIm = _doc.FittingWeightIm > 0;
        _selectedInterpolation = _doc.InterpolationFunction;
    ...
    private void InitializeInterpolationFunctionChoices()
    {
      ...
      var types = Altaxo.Main.Services.ReflectionService.GetNonAbstractSubclassesOf(typeof(Altaxo.Calc.Interpolation.IComplexInterpolation));
      ...
      InterpolationFunction0.SelectedValue = _selectedInterpolation?.GetType();
    }
    ...
    private void EhInterpolationFunctionChanged(Type type)
    {
      ...
      {
        _selectedInterpolation = (IComplexInterpolation)Activator.CreateInstance(type);
      }
    ...
    public override bool Apply(bool disposeController)
    {
      if (InterpolationDetails is not null)
      {
        ...
        _selectedInterpolation = (IComplexInterpolation)(InterpolationDetails.ModelObject);
      }
      ...
      _doc = _doc with
      {
        ...
        InterpolationFunction = _selectedInterpolation,
