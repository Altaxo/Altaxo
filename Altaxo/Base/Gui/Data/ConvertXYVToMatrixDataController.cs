...
namespace Altaxo.Gui.Data
{
  ...
  /// <summary>
  /// Provides the view contract for <see cref="ConvertXYVToMatrixDataController"/>.
  /// </summary>
  public interface IConvertXYVToMatrixDataView
  {
    /// <summary>
    /// Initializes the selectable X column list.
    /// </summary>
    /// <param name="list">The available X column choices.</param>
    void InitializeXColumn(SelectableListNodeList list);

    /// <summary>
    /// Initializes the selectable Y column list.
    /// </summary>
    /// <param name="list">The available Y column choices.</param>
    void InitializeYColumn(SelectableListNodeList list);
  ...
  /// <summary>
  /// Controller for editing <see cref="DataTableMultipleColumnProxy"/> data used to convert XYV data to a matrix.
  /// </summary>
  [ExpectedTypeOfView(typeof(IConvertXYVToMatrixDataView))]
  public class ConvertXYVToMatrixDataController : MVCANControllerEditOriginalDocBase<DataTableMultipleColumnProxy, IConvertXYVToMatrixDataView>
  {
    private SelectableListNodeList _choicesXColumn = new SelectableListNodeList();
    private SelectableListNodeList _choicesYColumn = new SelectableListNodeList();
    ...
    {
      _choicesXColumn = null;
      _choicesYColumn = null;
      _valueColumns = null;
    ...
    protected override void Initialize(bool initData)
    {
      ...
      if (initData)
      {
        ConvertXYVToMatrixDataAndOptions.EnsureCoherence(_doc, false);
    ...
    public override bool Apply(bool disposeController)
    {
      ...
      _doc.SetDataColumn(ConvertXYVToMatrixDataAndOptions.ColumnX, (DataColumn)_choicesXColumn.FirstSelectedNode.Tag);
      _doc.SetDataColumn(ConvertXYVToMatrixDataAndOptions.ColumnY, (DataColumn)_choicesYColumn.FirstSelectedNode.Tag);
      _doc.SetDataColumns(ConvertXYVToMatrixDataAndOptions.ColumnV, _valueColumns.Select(n => (IReadableColumnProxy)n.Tag));
    ...
    private void AddAllColumnsOfGroupToParticipatingColumns()
    {
      ...
      _doc.SetDataColumns(ConvertXYVToMatrixDataAndOptions.ColumnV, columnsInGroup);
    }
    ...
    private void ChangeDocumentToTableAndGroup(DataTable table, int groupNumber)
    {
      ...
      newDoc.EnsureExistenceOfIdentifier(ConvertXYVToMatrixDataAndOptions.ColumnX, 1); // because of the 1 argument, we can not set it below
      newDoc.EnsureExistenceOfIdentifier(ConvertXYVToMatrixDataAndOptions.ColumnY, 1); // because of the 1 argument, we can not set it below
      foreach (var identifier in new[] { ConvertXYVToMatrixDataAndOptions.ColumnV, ConvertXYVToMatrixDataAndOptions.ColumnX, ConvertXYVToMatrixDataAndOptions.ColumnY })
      {
