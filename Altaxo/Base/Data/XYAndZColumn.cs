...
namespace Altaxo.Data
{
  ...
  public class XYAndZColumn : IndependentAndDependentColumns
  {
    ...
    /// <summary>
    /// Initializes a new instance by copying the state from another <see cref="XYAndZColumn"/>.
    /// </summary>
    /// <param name="from">The source instance to copy.</param>
    public XYAndZColumn(XYAndZColumn from) : base(from)
    {
    }
    ...
    public XYAndZColumn(DataTable table, int groupNumber) : base(table, groupNumber, 2, 1)
    {
    }
    ...
    {
      get { return GetIndependentVariable(1); }
      set { SetIndependentVariable(1, value); }
    }

    /// <summary>
    /// Gets or sets the Y (dependent) column.
    /// </summary>
    public virtual IReadableColumn? ZColumn
    {
    ...
    {
      return string.Format("{0}(X), {1}(Y), {2}(Z)", _independentVariables[0].ToString(), _independentVariables[1].ToString(), _dependentVariables[0].ToString());
    }
    ...
    public (double[]? X, double[]? Y, double[]? Z, int RowCount) GetResolvedXYZData()
    {
      var result = GetResolvedData();
      return (result.Independent[0], result.Independent[1], result.Dependent[0], result.RowCount);
    }
    ...
    public string GetYName(int level)
    {
      ...
      else if (_independentVariables.Length > 1 && _independentVariables[1] is not null)
      {
        return _independentVariables[1].GetName(level) + " (broken)";
      }
      else
      {
        return " (broken)";
    ...
    /// <summary>
    /// Creates a new <see cref="XYAndZColumn"/> using existing proxies without cloning them.
    /// </summary>
    ...
    {
      return new XYAndZColumn(table, groupNumber, xCol, yCol, zCol);
    }
    ...
    public ColumnKind GetZKind()
    {
      if (ZColumn is null)
        return ColumnKind.V;
      var rootCol = IReadableColumn.GetRootDataColumn(ZColumn);
      if (rootCol is null)
        return ColumnKind.V;
      else
        return DataTable.DataColumns.GetColumnKind(rootCol);
    ...
    {
      return GetPropertyValueOfIndependentOrDependentVariable(isIndependent: true, 1, propertyName);
    }
    ...
    public AltaxoVariant GetZPropertyValue(string propertyName)
    {
