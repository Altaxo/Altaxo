...
namespace Altaxo.Data
{
  ...
  public abstract class DataTableMatrixProxyBase : Main.SuspendableDocumentNodeWithEventArgs, Main.ICopyFrom
  {
    ...
    protected List<IReadableColumnProxy> _columnHeaderColumns;
    ...
    /// <summary>
    /// Copies data from another instance of <see cref="DataTableMatrixProxyBase"/>.
    /// </summary>
    ...
    public bool CopyFrom(object obj)
    {
      ...
      if (obj is DataTableMatrixProxyBase from)
      {
        ...
        InternalSetColumnHeaderColumnsWithCloning(from._columnHeaderColumns);
    ...
    {
      var result = (DataTableMatrixProxyBase)Activator.CreateInstance(GetType());
    ...
    protected override IEnumerable<Main.DocumentNodeAndName> GetDocumentNodeChildrenWithName()
    {
      ...
      if (_columnHeaderColumns is not null)
      {
        for (int i = 0; i < _columnHeaderColumns.Count; ++i)
        {
          if (_columnHeaderColumns[i] is not null)
            yield return new Main.DocumentNodeAndName(_columnHeaderColumns[i], "ColumnHeaderColumn" + i.ToString(System.Globalization.CultureInfo.CurrentCulture));
        }
      }
...
#pragma warning disable CS8618 // Non-nullable field is uninitialized. Consider declaring as nullable.
    protected DataTableMatrixProxyBase()
    {
    }
#pragma warning restore CS8618 // Non-nullable field is uninitialized. Consider declaring as nullable.
    ...
    public DataTableMatrixProxyBase(DataTable table, IAscendingIntegerCollection selectedDataRows, IAscendingIntegerCollection selectedDataColumns, IAscendingIntegerCollection selectedPropertyColumns)
    {
      if (table is null)
    ...
    protected void InternalAddDataColumnNoClone(IReadableColumnProxy proxy)
    {
    ...
    private void InternalAddColumnHeaderColumnNoClone(IReadableColumnProxy proxy)
    {
      if (proxy is not null)
      {
        _columnHeaderColumns.Add(proxy);
        proxy.ParentObject = this;
      }
    }
    ...
    /// <summary>
    /// Removes the column header column proxy at index <paramref name="idx"/>, removing the Changed event handler.
    /// </summary>
    /// <param name="idx">The index.</param>
    private void InternalRemoveColumnHeaderColumnAt(int idx)
    {
      var col = _columnHeaderColumns[idx];
      _columnHeaderColumns.RemoveAt(idx);
      col?.Dispose();
    }

    private void InternalClearDataColumns()
    {
      var chc = _dataColumns;
      _dataColumns = new List<IReadableColumnProxy>();
      foreach (var ele in chc)
        ele?.Dispose();
    }

    private void InternalClearColumnHeaderColumns()
    {
      var chc = _columnHeaderColumns;
      _columnHeaderColumns = new List<IReadableColumnProxy>();
      EnumerableExtensions.DisposeElements(chc);
    }
    ...
    private void InternalSetColumnHeaderColumnsWithCloning(IEnumerable<IReadableColumnProxy> fromList)
    {
      ...
      foreach (var fromMember in fromList)
      {
        var clone = (IReadableColumnProxy)fromMember.Clone();
        clone.ParentObject = this;
        _columnHeaderColumns.Add(clone);
      }
    ...
    private void TryGetDataTableProxyFromColumns()
    {
      ...
      foreach (var colproxy in _columnHeaderColumns)
      {
        col = colproxy.Document() as DataColumn;
        if (col is not null)
        {
      ...
      foreach (var colproxy in new IReadableColumnProxy[] { _rowHeaderColumn })
      {
        col = colproxy.Document() as DataColumn;
        if (col is not null)
        {
    ...
    public void ForEachColumnHeaderElementDo(Action<IReadableColumn, int> action)
    {
      ...
      var col = _columnHeaderColumns.Count == 0 ? null : _columnHeaderColumns[0]?.Document();
    ...
    public bool TryGetColumnHeaderIncrement(out double incrementValue, [MaybeNullWhen(true)] out string errorOrWarningMessage)
    {
      ...
      if (_columnHeaderColumns.Count == 0)
      {
        incrementValue = 1;
        errorOrWarningMessage = "No column header column present - increment value can not be determined";
        return false;
      }

      return TryGetColumnDataIncrement(_columnHeaderColumns[0], "column", _participatingDataColumns, out incrementValue, out errorOrWarningMessage);
    }
