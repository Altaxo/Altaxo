...
namespace Altaxo.Graph.Scales.Deprecated
{
  ...
  public class DateTimeScale : Scale
  {
    ...
    private struct SpanCompound
    {
      ...
      private DateTime RoundUpSpan(DateTime d)
      {
      ...
      private DateTime RoundDownSpan(DateTime d)
      {
        long dd = (d - DateTime.MinValue).Ticks;
        long rn = _span.Ticks;
        long re = dd % rn;
      ...
      private DateTime RoundUpYears(DateTime d)
      {
        int m = (int)_span.Ticks;
        return new DateTime(Altaxo.Calc.Rounding.RoundUp(d.Year + 1, m), 1, 1);
      }

      private DateTime RoundDownYears(DateTime d)
      {
        int m = (int)_span.Ticks;
    ...
    private static SpanCompound CalculateYearTicks(TimeSpan span)
    {
    ...
    private const double _DaysPerMonth = 31;

    private static SpanCompound CalculateMonthTicks(TimeSpan span)
    {
