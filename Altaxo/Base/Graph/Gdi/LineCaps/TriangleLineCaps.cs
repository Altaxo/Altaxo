...
namespace Altaxo.Graph.Gdi.LineCaps
{
  ...
  public class TriangleOLineCap : LineCapExtension
  {
    ...
    CustomLineCap GetClone(Pen pen, float size)
    {
      ...
      hPath.AddPolygon(new PointF[]{
        new PointF(0,-endPoint*0.866f),
        new PointF(-endPoint/2,0),
        new PointF(endPoint/2,0)
      });
      CustomLineCap clone = new CustomLineCap(null, hPath, LineCap.Flat, endPoint*0.866f ); // we set the stroke path only
  ...
  public class TriangleFLineCap : LineCapExtension
  {
    ...
    CustomLineCap GetClone(Pen pen, float size)
    {
      ...
      CustomLineCap clone = new CustomLineCap(null, hPath, LineCap.Flat, 0); // we set the stroke path only
      clone.SetStrokeCaps(LineCap.Flat, LineCap.Flat);
      return clone;
    }
