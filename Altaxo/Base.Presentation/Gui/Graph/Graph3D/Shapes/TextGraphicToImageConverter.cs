...
namespace Altaxo.Gui.Graph.Graph3D.Shapes
{
  ...
  public class TextGraphicToImageConverter : IMultiValueConverter, IDisposable
  {
    ...
    {
      if(values is not null && values.Length>=3 && values[0] is TextGraphic textGraphic3D && values[1] is double dwidth && values[2] is double dheight)
      {

        using var textGraphic = new Altaxo.Graph.Gdi.Shapes.TextGraphic(textGraphic3D.GetPropertyContext())
        {
          Text = textGraphic3D.Text,
          Color = textGraphic3D.Color,
          Font = textGraphic3D.Font.Font,
          ParentObject = textGraphic3D.ParentObject
        };
