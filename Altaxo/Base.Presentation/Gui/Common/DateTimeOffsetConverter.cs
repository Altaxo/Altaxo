...
{
  /// <summary>
  /// Converts between <see cref="DateTimeOffset"/> values and their text representation for data binding.
  /// </summary>
  public class DateTimeOffsetConverter : ValidationRule, IValueConverter
  {
    ...
    private DateTimeOffset? _lastConvertedValue;
    ...
    private ValidationResult ConvertAndValidate(object value, out DateTimeOffset result)
    {
      var s = (string)value;

      if (_lastConvertedValue is not null && s == _lastConvertedString)
      {
        result = (DateTimeOffset)_lastConvertedValue;
        return ValidateSuccessfullyConvertedValue(result);
      }

      if (DateTimeOffset.TryParse(s, _conversionCulture, System.Globalization.DateTimeStyles.AssumeLocal, out result))
      {
        return ValidateSuccessfullyConvertedValue(result);
      }

      return new ValidationResult(false, "String could not be converted to a DateTimeOffset");
    }

    private ValidationResult ValidateSuccessfullyConvertedValue(DateTimeOffset result)
    {
